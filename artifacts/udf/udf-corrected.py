...
# =============================================================================
# UPDATE EMPLOYEES WITH PIPELINE TRIGGER - Version with automatic sync
# =============================================================================
@udf.connection(argName="sqlDB", alias="HRData")  # ← FIXED: Added @ symbol
@udf.function()
def update_employees(sqlDB: fn.FabricSqlConnection, employeeId: int, employeeName: str, modifiedBy: str) -> str:
    """
    Update employee record in Fabric SQL Database and trigger sync pipeline via stored procedure.
    
    The stored procedure (usp_UpdateEmployee) should handle both the update AND pipeline trigger.
    This keeps the logic in SQL where sp_invoke_external_rest_endpoint is available.
    ...
    try:
        ...
        # Execute stored procedure (which should trigger pipeline internally)
        sql = """
        EXEC dbo.usp_UpdateEmployee 
            @EmployeeID = ?, 
            @EmployeeName = ?, 
        ...
        # Return success message
        # Note: If your stored procedure triggers the pipeline, include that in the message
        return f"✅ Successfully updated {employeeName} (ID: {employeeId}) by {modifiedBy} | 🔄 Sync pipeline triggered"
